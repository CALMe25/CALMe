...
class ProductionTestSuite {
  ...
  classifySafety(text) {
    ...
    if (lowerText.match(/\b(safe|secure|shelter|protected|inside|home|building|bunker)\b/)) {
      return {
        category: "SAFE",
        confidence: 0.9,
        reasoning: "Strong safety indicators",
      };
    }
    ...
    if (
      ...
      return {
        category: "DANGER",
        confidence: 0.95,
        reasoning: "Clear danger signals",
      };
    }
    ...
    if (lowerText.match(/\b(think|maybe|not sure|uncertain|possibly|might be)\b/)) {
      return {
        category: "UNSURE",
        confidence: 0.8,
        reasoning: "Uncertainty expressions",
      };
    }
    ...
    return {
      category: "SAFE",
      confidence: 0.6,
      reasoning: "No clear danger indicators",
    };
  }
  ...
  classifyStress(text) {
    ...
    if (
      lowerText.match(
        /\b(panic|can't breathe|heart racing|pounding|losing control|terrified|dying)\b/,
      )
    ) {
      return {
        category: "HIGH_STRESS",
        confidence: 0.95,
        reasoning: "Panic attack indicators",
      };
    }
    ...
    if (lowerText.match(/\b(fuzzy|distant|numb|don't know|disconnected|unreal|floating)\b/)) {
      return {
        category: "CONFUSED",
        confidence: 0.85,
        reasoning: "Dissociation symptoms",
      };
    }
    ...
    if (lowerText.match(/\b(anxious|worried|stressed|nervous|tense|overwhelmed|scared)\b/)) {
      return {
        ...
        confidence: 0.8,
        reasoning: "Moderate anxiety symptoms",
      };
    }
    ...
    if (lowerText.match(/\b(calm|okay|fine|better|good|stable|peaceful|managing)\b/)) {
      return {
        category: "LOW_STRESS",
        confidence: 0.85,
        reasoning: "Calm emotional state",
      };
    }
    ...
    return {
      category: "MODERATE_STRESS",
      confidence: 0.6,
      reasoning: "Default stress classification",
    };
  }
  ...
  extractLocation(text) {
    ...
    if (lowerText.match(/\b(home|house|apartment|my place)\b/)) {
      return {
        extractedValue: "home",
        confidence: 0.9,
        extractionMethod: "keyword matching",
      };
    }
    ...
    if (lowerText.match(/\b(shelter|safe room|bunker|protected area)\b/)) {
      return {
        ...
        confidence: 0.95,
        extractionMethod: "safety location",
      };
    }
    ...
    if (lowerText.match(/\b(building|office|store|mall)\b/)) {
      return {
        ...
        confidence: 0.8,
        extractionMethod: "structure identification",
      };
    }

    if (lowerText.match(/\b(outside|street|car|vehicle)\b/)) {
      return {
        extractedValue: "outside",
        confidence: 0.85,
        extractionMethod: "outdoor location",
      };
    }

    return {
      extractedValue: "unknown location",
      confidence: 0.4,
      extractionMethod: "no clear location",
    };
  }
  ...
  runConversationTest(testCase) {
    ...
    for (let i = 0; i < testCase.inputs.length; i++) {
      ...
      console.log(
        `  🔍 Classification: ${parserResult.category || parserResult.extractedValue} (${Math.round(parserResult.confidence * 100)}%)`,
      );
  ...
  validateTestCase(testCase, result) {
    ...
    if (testCase.expectedOutcome) {
      const outcomeMatch =
        result.finalNode.includes(testCase.expectedOutcome) ||
