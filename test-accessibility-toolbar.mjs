...
async function testAccessibilityToolbar() {
  ...
  try {
    ...
    await page.screenshot({ path: join(SCREENSHOTS_DIR, '1-initial-page.png'), fullPage: true });
    ...
    await page.screenshot({ path: join(SCREENSHOTS_DIR, '2-toolbar-initial-language.png'), fullPage: true });
    ...
    if (languageSwitcherClicked) {
      ...
      await page.screenshot({ path: join(SCREENSHOTS_DIR, '3-language-dropdown.png'), fullPage: true });
      ...
      const hebrewClicked = await page.evaluate(() => {
        ...
        const hebrewItem = menuItems.find(item => item.textContent.includes('עברית'));
        if (hebrewItem) {
          hebrewItem.click();
          return true;
        }
    ...
    await page.screenshot({ path: join(SCREENSHOTS_DIR, '4-page-after-language-switch.png'), fullPage: true });
    ...
    await page.screenshot({ path: join(SCREENSHOTS_DIR, '5-toolbar-after-language-switch.png'), fullPage: true });
    ...
    if (toolbarLang && toolbarLangAfter) {
      ...
      const textChanged = toolbarLang.resetButtonText !== toolbarLangAfter.resetButtonText;
      if (textChanged) {
